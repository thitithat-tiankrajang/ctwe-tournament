...
export function RankingGrid({ ranked, storageKey, resetKey, emptyText = "ไม่พบผู้เล่นตามตัวกรอง", onRowClick, activeId }: {
  ...
  return <DataGrid columns={columns} rows={rows} getRowKey={({ player }) => player.id} storageKey={storageKey} resetKey={resetKey} unit="คน" emptyText={emptyText} onRowClick={onRowClick ? (row) => onRowClick(row.player) : undefined} rowClassName={activeId ? (row) => row.player.id === activeId ? "egrid-row--active" : undefined : undefined} />;
}
...
export function PairingGrid({ pairings, players, storageKey, resetKey, emptyText = "ไม่พบคู่ตามตัวกรอง" }: {
  ...
  const columns: DataColumn<Pairing>[] = [
    ...
    { key: "id1", label: "รหัสฝ่ายที่ 1", min: 80, width: 116, cellClassName: "cell-id", value: (pairing) => playerOf(pairing.playerOneId)?.id ?? "—", render: (pairing) => playerOf(pairing.playerOneId)?.id ?? "—" },
    { key: "name1", label: "ชื่อ - นามสกุล", min: 120, width: 184, value: (pairing) => fullName(pairing.playerOneId), render: (pairing) => <span title={fullName(pairing.playerOneId)}>{fullName(pairing.playerOneId)}</span> },
    { key: "school1", label: "โรงเรียน/สถาบัน", min: 110, width: 174, value: (pairing) => playerOf(pairing.playerOneId)?.school ?? "—", render: (pairing) => <span title={playerOf(pairing.playerOneId)?.school}>{playerOf(pairing.playerOneId)?.school ?? "—"}</span> },
    ...
    { key: "id2", label: "รหัสฝ่ายที่ 2", min: 80, width: 116, cellClassName: "cell-id", value: (pairing) => playerOf(pairing.playerTwoId)?.id ?? "—", render: (pairing) => playerOf(pairing.playerTwoId)?.id ?? "—" },
    { key: "name2", label: "ชื่อ - นามสกุล", min: 120, width: 184, value: (pairing) => fullName(pairing.playerTwoId), render: (pairing) => <span title={fullName(pairing.playerTwoId)}>{fullName(pairing.playerTwoId)}</span> },
    { key: "school2", label: "โรงเรียน/สถาบัน", min: 110, width: 174, value: (pairing) => playerOf(pairing.playerTwoId)?.school ?? "—", render: (pairing) => <span title={playerOf(pairing.playerTwoId)?.school}>{playerOf(pairing.playerTwoId)?.school ?? "—"}</span> },
  ];
  return <DataGrid columns={columns} rows={pairings} getRowKey={(pairing) => pairing.id} storageKey={storageKey} resetKey={resetKey} unit="คู่" emptyText={emptyText} />;
}
