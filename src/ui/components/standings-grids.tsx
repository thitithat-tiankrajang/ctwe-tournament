...
export function RankingGrid({ ranked, storageKey, resetKey, emptyText = "ไม่พบผู้เล่นตามตัวกรอง", onRowClick, activeId }: {
  ...
  const columns: DataColumn<{ player: Player; rank: number }>[] = [
    ...
    { key: "school", label: "โรงเรียน/สถาบัน", min: 120, width: 200, cellClassName: "cell-person-school cell-ranking-school", value: ({ player }) => player.school, render: ({ player }) => <span title={player.school}>{player.school}</span> },
