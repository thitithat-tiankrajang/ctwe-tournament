...
export function PlayerHistoryTable({ card, players, playerId }: { card: TournamentCard; players: Map<string, Player>; playerId: string }) {
  ...
  return <DataGrid ariaLabel="ประวัติการแข่งขันของนักกีฬา" columns={columns} rows={history} getRowKey={(row) => `${row.game}`} storageKey={`${card.id}:history:detail`} emptyText="ยังไม่มีประวัติการเล่นที่เผยแพร่" />;
}
