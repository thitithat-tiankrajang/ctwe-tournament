...
>
> แผนแบ่งเป็น 2 เฟส — **ทำเสร็จแล้วทั้งสองเฟส**:
> - **Phase 1 (migration `V19`)**: ตัดข้อมูลซ้ำซ้อน/คอลัมน์ที่ไม่มีใครอ่านออก โดย **ไม่เปลี่ยน API และไม่เสี่ยงกับข้อมูล** — ลดได้ ~35–55% ทันที
> - **Phase 2 (migration `V20`)**: เปลี่ยนระบบ key ทั้งหมดเป็น key เล็ก (smallint/composite) และรวม `match_results` เข้า `matches` — ลดรวม ~70–85% · ผ่านการทดสอบ migration กับข้อมูลจริงแบบ end-to-end แล้ว (ดู §6)
...
### Phase 2 — `V20__phase2_rekey.sql` (ทำแล้วใน repo นี้)

Migration เป็นแบบ **copy-transform ใน transaction เดียว**: สร้างตารางใหม่ข้างตารางเดิม → คัดลอกข้อมูลพร้อมแปลง key → **assert จำนวนแถวและจำนวน reference ทุกตารางก่อน drop ของเดิม** — ถ้าไม่ตรงแม้แถวเดียว migration ทั้งไฟล์ rollback และข้อมูลเดิมไม่ถูกแตะเลย (ตามนโยบาย data ห้ามหาย)

สิ่งที่เปลี่ยนตาม ER §3: players → `(card_id, code SMALLINT)`, matches รวม `match_results` + composite PK `(card_id, game_number, table_number)`, snapshots → `(card_id, snapshot_no)` + `game_from/game_to`, `competition_tables`+`table_players` → `table_seats`, standings → `(card_id, player_code)`, audit id → BIGINT IDENTITY, `matches.result_type` → CHAR(1) W/D/P

**สิ่งที่จงใจเบี่ยงจาก spec เดิม (เหตุผล: ตารางเล็กจิ๋ว อ่านง่ายสำคัญกว่า byte):**
- `games.status`, `pairing_rules.rule_type`, `tournament_cards.status/runtime_stage/final_type` คงเป็น VARCHAR ค่าเต็ม
- `standings.rank` คงไว้ (ใช้เรียง seed รอบชิง)

**รูปแบบ id ฝั่ง API (frontend ใช้เป็น opaque string อยู่แล้ว จึงไม่ต้องแก้ frontend เลย):**
- ผู้เล่น: `"P001"` (เท่าเดิม — ตอนนี้ derive จาก smallint)
- คู่แข่งขัน (match/pairing id): `"g3t12"` = เกม 3 โต๊ะ 12 (เดิมเป็น UUID)
- snapshot: `"s1"`, `"s2"` · เกม/โต๊ะ: เลขตรงๆ

**การทดสอบที่ทำแล้ว (2026-07-02, Postgres 17 จริงใน Docker):**
1. Boot โค้ด production (V19) → สร้างข้อมูลจริงผ่าน API: 4 การ์ด 4 สถานะ (จบแล้ว+รอบชิง / กลางทาง มี bye+ลงดาบ+snapshot / ลงทะเบียน+ลบคน / ผลค้างยังไม่ publish) — ผู้เล่น 23, คู่แข่ง 24, snapshot 4, audit 69 แถว
2. สลับเป็นโค้ด V20 → Flyway แปลงข้อมูลผ่านทุก assertion
3. เทียบ JSON ทุก endpoint ก่อน/หลัง: **ข้อมูลตรงกัน 100% ทุก field** (ผู้เล่น ผล อันดับ snapshot รอบชิง audit)
4. ทดสอบเขียนต่อบนข้อมูลที่แปลงแล้ว: กรอกผลต่อจนจบการ์ด, override, penalty, ลบ/เพิ่มผู้เล่น+renumber, การ์ดใหม่สนามคี่ (bye + deferred swiss), archive → Excel ถูกต้อง — ผ่านทั้งหมด
**ผลรวม: ลด ~70–85% และ write path เร็วขึ้นจาก index ที่เหลือชุดเดียวต่อตาราง**
